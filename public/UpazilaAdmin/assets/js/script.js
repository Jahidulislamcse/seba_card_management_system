...
// summery report (union select) start
const select_btns = document.querySelector('.select-btns');
if(select_btns){
    const btns = document.querySelector('.btns');
    const all_report = document.querySelector('.all_report');
    const union_report = document.querySelector('.union_report');
    const buttons = document.querySelectorAll('.buttons');

    select_btns.addEventListener("click", (() =>{
        btns.classList.toggle('active-tab-btn');        
    }));

    buttons.forEach((btn, index) =>{
        btn.addEventListener("click", (()=>{
            btns.classList.remove('active-tab-btn');  
            select_btns.innerHTML = btn.innerText + `<img src="assets/img/arrow down.png" alt="arrow icon">`;

            if(btn.innerText == "সকল"){
                all_report.classList.add('active_report');
                union_report.classList.remove('active_report');
            }
            else{
                all_report.classList.remove('active_report');
                union_report.classList.add('active_report');
            }
        }))
    })
}
// summery report (union select) end
