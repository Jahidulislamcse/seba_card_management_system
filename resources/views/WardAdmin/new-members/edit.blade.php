...
<form class="member-add-form" action="{{ route('ward.new-members.update', $data->id) }}" method="POST"
    ...
    <div class="input-group mb-2">
        ...
        <select class="input-box form-control shadow-none" name="card_id" id="card_id" required disabled>
            ...
            <option value="{{ $card->id }}"
                {{ old('card_id', optional($data->card)->id ?? '') == $card->id ? 'selected' : '' }}>
                {{ $card->card_number }}
            </option>
            @endforeach
        </select>
    ...
    <div class="input-group mb-2">
        ...
        <select name="upazila_id" id="upozila" class="input-box select-box" required>
            <option value="">উপজেলা নির্বাচন করুন</option>
            @foreach ($upazila as $up)
            <option value="{{ $up->id }}" {{ $data->upazila_id == $up->id ? 'selected' : '' }}>{{ $up->name }}</option>
            @endforeach
        </select>
        @error('upazila_id')
        <p class="text-danger m-0">{{ $message }}</p>
...
<script>
    $(document).ready(function() {
        ...
        }; // Start from 1 since the first one is already there
